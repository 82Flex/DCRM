...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunSQL(
            "CREATE VIEW `package_view` AS SELECT `WEIPDCRM_version`.`id`, `WEIPDCRM_version`.`c_name`, `WEIPDCRM_version`.`created_at`, `WEIPDCRM_version`.`c_package`, `WEIPDCRM_version`.`c_version`, `WEIPDCRM_version`.`enabled` FROM `WEIPDCRM_version` WHERE `WEIPDCRM_version`.`enabled` = TRUE GROUP BY `WEIPDCRM_version`.`c_package` DESC;"
        ),
        migrations.RunSQL(
