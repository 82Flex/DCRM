        ...
        # do confirmed action
            ...
            if request.POST['action'] == 'delete':
                for job_id in job_ids:
                    ...
                    queue.connection._lrem(queue.key, 0, job.id)
                    job.delete()
                messages.info(request, _('You have successfully deleted %s jobs!') % len(job_ids))
            elif request.POST['action'] == 'requeue':
                for job_id in job_ids:
                    requeue_job(job_id, connection=queue.connection)
                messages.info(request, _('You have successfully requeued %d jobs!') % len(job_ids))
